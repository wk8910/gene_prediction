...
while (<BOUT>) {
    ...
    if (/^Query=/) {
        ...
        if (/^\s+\(([0-9\,]+) letters\)/) {
            ...
            if ($tmpqline =~ /^Query=\s+(\S+)/) {    # modified on 04/12/2002
                $qid = $1;
            } else {
                $qid = "Query";
            }
        ...
        $qid = "Query";
        $nali = 0;
    } elsif (/^Matrix: / && $nali == 0 && !$nhfrag && !$qini) {
...
#t    print OUTFILE "normh1x2 = $normh1x2\n";
